...
class CustomCompleter(Completer):
    ...
    DATE_MAP: dict[str, date] = {
        ...
        "tuesday": date.today() + timedelta(days=((8 - date.today().weekday()) % 7) or 7),
        "wednesday": date.today() + timedelta(days=((9 - date.today().weekday()) % 7) or 7),
        "thursday": date.today() + timedelta(days=((10 - date.today().weekday()) % 7) or 7),
        "friday": date.today() + timedelta(days=((11 - date.today().weekday()) % 7) or 7),
        "saturday": date.today() + timedelta(days=((12 - date.today().weekday()) % 7) or 7),
        "sunday": date.today() + timedelta(days=((13 - date.today().weekday()) % 7) or 7)
    }
...
if __name__ == "__main__":
    ...
    result = choice(
        message="Select an option:", options=[(1, "Add Task"), (2, "View Tasks"), (3, "Get task from Apriltag")]
    )
    if result == 1:
        ...
        completer = CustomCompleter(categories, ["tomorrow", "today", ])
        ...
        try:
            ...
            if task is not None:
                ...
                print_formatted_text(
                    HTML(f'<white bg="red">No task found for Apriltag ID: {tag_id}</white>')
                )
        except ValueError:
            print_formatted_text(
                HTML(f'<white bg="red">Invalid Apriltag ID entered: {tag_id_str}</white>')
            )
